...
// fileTypeExtensions references the pkg-level map
...
func matchesFileType(path string, include, exclude []string) bool {
	return pkgrg.MatchesFileType(path, include, exclude)
}

func matchesGlob(path string, patterns []string) bool {
	return pkgrg.MatchesGlob(path, patterns)
}

func isBinary(data []byte) bool {
	return pkgrg.IsBinary(data)
}
