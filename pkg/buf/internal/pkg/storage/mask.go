...
func (r *maskReadBucketCloser) matchPath(path string) bool {
	...
	for _, excludePrefix := range r.excludePrefixes {
		...
		if normalpath2.EqualsOrContainsPath(excludePrefix, path, normalpath2.Relative) {
			return false
		}
	}
	...
	for _, includePrefix := range r.includePrefixes {
		...
		if normalpath2.EqualsOrContainsPath(includePrefix, path, normalpath2.Relative) {
			return true
		}
	}
...
func normalizeValidateAndCompactPrefixes(prefixes []string) ([]string, error) {
	...
	for _, prefix := range normalized[1:] {
		if !normalpath2.EqualsOrContainsPath(reduced[len(reduced)-1], prefix, normalpath2.Relative) {
