...
func TestMaskReadBucket_InvalidPrefixes(t *testing.T) {
	...
	_, err = storage2.MaskReadBucket(readWriteBucket, []string{}, []string{"../invalid"})
...
func TestMaskReadBucket_PrefixCompaction(t *testing.T) {
	...
	filteredBucket2, err := storage2.MaskReadBucket(
		readWriteBucket,
...
func testMaskReadBucket(
	t *testing.T,
	delegate storage2.ReadWriteBucket,
	testFiles map[string]string,
	...
	filteredBucket, err := storage2.MaskReadBucket(delegate, includePrefixes, excludePrefixes)
	require.NoError(t, err)
...
func testMaskReadBucket_WalkOptimization(t *testing.T,
	...
	filteredBucket, err := storage2.MaskReadBucket(trackingBucket, includePrefixes, []string{})
	require.NoError(t, err)
	err = filteredBucket.Walk(t.Context(), walkPrefix, func(storage2.ObjectInfo) error {
		return nil
	})
	require.NoError(t, err)
...
func (w *walkTrackingBucket) Get(ctx context.Context, path string) (storage2.ReadObjectCloser, error) {
	return w.delegate.Get(ctx, path)
}

func (w *walkTrackingBucket) Stat(ctx context.Context, path string) (storage2.ObjectInfo, error) {
	return w.delegate.Stat(ctx, path)
}

func (w *walkTrackingBucket) Walk(ctx context.Context, prefix string, f func(storage2.ObjectInfo) error) error {
