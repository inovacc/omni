...
func (r *reader) getArchiveBucket(
	...
	return getReadBucketCloserForBucket(
		ctx,
		r.logger,
		storage2.NopReadBucketCloser(readWriteBucket),
...
func getGitURL(gitRef GitRef) (string, error) {
	...
	case GitSchemeLocal:
		absPath, err := filepath.Abs(normalpath2.Unnormalize(gitRef.Path()))
		if err != nil {
			return "", err
		}
...
func getReadBucketCloserForBucket(
	ctx context.Context,
	logger *slog.Logger,
	inputBucket storage2.ReadBucketCloser,
	...
	if bucketTargeting.ControllingWorkspace() != nil && bucketTargeting.ControllingWorkspace().Path() != "." {
		...
		inputBucket = storage2.MapReadBucketCloser(
			inputBucket,
...
func getReadWriteBucketForOS(
	...
	if controllingWorkspace := osRootBucketTargeting.ControllingWorkspace(); controllingWorkspace != nil {
		if filepath.IsAbs(normalpath2.Unnormalize(inputDirPath)) {
			bucketPath = normalpath2.Join(fsRoot, osRootBucketTargeting.ControllingWorkspace().Path())
		} else {
			// Relative input dir
		...
		if filepath.IsAbs(normalpath2.Unnormalize(inputDirPath)) {
			...
			if filepath.IsLocal(normalpath2.Unnormalize(inputDirPath)) {
				...
				bucketPath, err = normalpath2.Rel(pwdFSRelPath, fsRootInputSubDirPath)
				if err != nil {
					return nil, nil, err
				}
			}
...
func fsRootAndFSRelPathForPath(path string) (string, string, error) {
	absPath, err := normalpath2.NormalizeAndAbsolute(path)
	if err != nil {
		return "", "", err
	}
...
func mapTargetPathsAndTargetExcludePathsForArchiveAndGitRefs(
	...
	return xslices.Map(
			...
			func(targetPath string) string {
				return normalpath2.Join(inputSubDirPath, targetPath)
			},
		),
		xslices.Map(
			targetExcludePaths,
			func(targetExcludePath string) string {
				return normalpath2.Join(inputSubDirPath, targetExcludePath)
			},
		)
}
