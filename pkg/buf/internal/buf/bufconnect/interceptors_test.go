...
func TestCLIWarningInterceptor(t *testing.T) {
	...
	logger, err := slogapp.NewLogger(&buf, appext2.LogLevelWarn, appext2.LogFormatText)
	require.NoError(t, err)
	// testing valid warning message
	_, err = NewCLIWarningInterceptor(appext2.NewLoggerContainer(logger))(func(ctx context.Context, req connect.AnyRequest) (connect.AnyResponse, error) {
