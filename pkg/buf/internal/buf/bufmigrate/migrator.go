...
func (m *migrator) getMigrateBuilder(
	...
	// in a buf.yaml v2.
	workspaceDirPaths, err := xslices.MapError(workspaceDirPaths, normalpath2.NormalizeAndValidate)
	if err != nil {
		return nil, err
	}

	moduleDirPaths, err = xslices.MapError(moduleDirPaths, normalpath2.NormalizeAndValidate)
	if err != nil {
		return nil, err
	}
	// This does mean that buf.gen.yamls need to be under the directory this is run at, but this is OK.
	bufGenYAMLFilePaths, err = xslices.MapError(bufGenYAMLFilePaths, normalpath2.NormalizeAndValidate)
	if err != nil {
		return nil, err
	}
...
func (m *migrator) migrate(ctx context.Context, bucket storage2.WriteBucket, migrateBuilder *migrateBuilder) (retErr error) {
...
func (m *migrator) getOriginalAndAddedFileBuckets(
	ctx context.Context,
	migrateBuilder *migrateBuilder,
) (_ storage2.ReadBucket, _ storage2.ReadBucket, retErr error) {
...
func (m *migrator) getModuleToRefToCommit(
	...
	for i, moduleRef := range moduleRefs {
		...
		// We know that the ModuleKeys and Commits match up with the ModuleRefs via the definition
