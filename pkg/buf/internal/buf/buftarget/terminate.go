...
func terminateAtControllingWorkspace(
	...
	if bufYAMLExists && bufYAMLFile.FileVersion() == bufconfig2.FileVersionV2 {
		...
		for _, moduleConfig := range bufYAMLFile.ModuleConfigs() {
			...
			if normalpath2.EqualsOrContainsPath(moduleConfig.DirPath(), relInputPath, normalpath2.Relative) {
				return newControllingWorkspace(prefix, nil, bufYAMLFile), nil
			}
			...
			if normalpath2.EqualsOrContainsPath(relInputPath, moduleConfig.DirPath(), normalpath2.Relative) {
				return newControllingWorkspace(prefix, nil, bufYAMLFile), nil
			}
		}
	}
	if bufWorkYAMLExists {
		...
		for _, dirPath := range bufWorkYAMLFile.DirPaths() {
			...
			// in a module path.
			if normalpath2.EqualsOrContainsPath(dirPath, relInputPath, normalpath2.Relative) {
				return newControllingWorkspace(prefix, bufWorkYAMLFile, nil), nil
			}
		}
	}
