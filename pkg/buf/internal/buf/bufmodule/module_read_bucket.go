...
import (
	...
	"github.com/inovacc/omni/pkg/buf/pkg/cache"
	...
	"github.com/inovacc/omni/pkg/buf/internal/buf/bufprotocompile"
)
...
func ModuleReadBucketToStorageReadBucket(bucket ModuleReadBucket) storage2.ReadBucket {
...
func GetDocStorageReadBucket(ctx context.Context, bucket storage2.ReadBucket) (storage2.ReadBucket, error) {
...
//
// This is needed for i.e. using RootToExcludes in NewWorkspaceForBucket.
func GetLicenseStorageReadBucket(ctx context.Context, bucket storage2.ReadBucket) (storage2.ReadBucket, error) {
	// Store the license file in a new memory bucket for performance reasons.
	content, err := storage2.ReadPath(ctx, bucket, licenseFilePath)
	if err != nil {
...
type moduleReadBucket struct {
	getBucket func() (storage2.ReadBucket, error)
	module    Module
...
func newModuleReadBucketForModule(
	...
	syncOnceValuesGetBucketWithStorageMatcherApplied func() (storage2.ReadBucket, error),
	module Module,
	targetPaths []string,
...
func (b *moduleReadBucket) WalkFileInfos(
	...
	targetFileWalkFunc := func(objectInfo storage2.ObjectInfo) error {
		fileInfo, err := b.getFileInfo(ctx, objectInfo)
		if err != nil {
			return err
		}
...
func (b *moduleReadBucket) getFileInfo(ctx context.Context, objectInfo storage2.ObjectInfo) (FileInfo, error) {
...
func (b *moduleReadBucket) getFileInfoUncached(ctx context.Context, objectInfo storage2.ObjectInfo) (FileInfo, error) {
...
func (b *moduleReadBucket) getIsTargetFileForPathUncached(ctx context.Context, path string) (bool, error) {
	...
	case len(b.targetPathMap) == 0 && len(b.targetExcludePathMap) != 0:
		...
		return !normalpath2.MapHasEqualOrContainingPath(b.targetExcludePathMap, path, normalpath2.Relative), nil
	case len(b.targetPathMap) != 0 && len(b.targetExcludePathMap) == 0:
		// We only have paths, no exclude paths.
		return normalpath2.MapHasEqualOrContainingPath(b.targetPathMap, path, normalpath2.Relative), nil
	default:
		// We have both paths and exclude paths.
		return normalpath2.MapHasEqualOrContainingPath(b.targetPathMap, path, normalpath2.Relative) &&
			!normalpath2.MapHasEqualOrContainingPath(b.targetExcludePathMap, path, normalpath2.Relative), nil
	}
}
...
func (s *storageReadBucket) Get(ctx context.Context, path string) (storage2.ReadObjectCloser, error) {
	return s.delegate.GetFile(ctx, path)
}

func (s *storageReadBucket) Stat(ctx context.Context, path string) (storage2.ObjectInfo, error) {
	return s.delegate.StatFileInfo(ctx, path)
}

func (s *storageReadBucket) Walk(ctx context.Context, prefix string, f func(storage2.ObjectInfo) error) error {
	prefix, err := normalpath2.NormalizeAndValidate(prefix)
	if err != nil {
		return err
	}
