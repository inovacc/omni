...
func GetConfigForBucket(ctx context.Context, readBucket storage2.ReadBucket, options ...ConfigOption) (*Config, error) {
...
func ExistingConfigFilePath(ctx context.Context, readBucket storage2.ReadBucket) (string, error) {
	for _, configFilePath := range AllConfigFilePaths {
		exists, err := storage2.Exists(ctx, readBucket, configFilePath)
		if err != nil {
			return "", err
		}
		if exists {
