...
type Image interface {
	...
	Resolver() protoencoding2.Resolver
...
func NewImageForProto(protoImage *imagev1.Image, options ...NewImageForProtoOption) (Image, error) {
	...
	resolver := protoencoding2.NewLazyResolver(protoImage.GetFile()...)
...
func reparseImageProto(protoImage *imagev1.Image, resolver protoencoding2.Resolver, computeUnusedImports bool) error {
...
func appendWellKnownTypeImageFileInfos(
	...
	// Sorted.
	wktObjectInfos, err := storage2.AllObjectInfos(ctx, wktBucket, "")
	if err != nil {
		return nil, err
	}
