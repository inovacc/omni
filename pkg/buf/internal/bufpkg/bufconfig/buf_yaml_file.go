...
func (c *bufYAMLFile) ConfiguredDepModuleRefs() []bufparse2.Ref {
		...
		for _, externalModule := range externalModules {
			...
			relIncludes, err := xslices.MapError(
				...
				func(normalInclude string) (string, error) {
					...
					return normalpath2.Rel(dirPath, normalInclude)
				},
			)
			if err != nil {
				return nil, err
			...
			relExcludes, err := xslices.MapError(
				...
				func(normalExclude string) (string, error) {
					normalExclude, err := normalpath2.NormalizeAndValidate(normalExclude)
					if err != nil {
						// user error
					...
					return normalpath2.Rel(dirPath, normalExclude)
				},
			)
			if err != nil {
				return nil, err
...
func writeBufYAMLFile(writer io.Writer, bufYAMLFile BufYAMLFile) error {
	...
	case FileVersionV1Beta1, FileVersionV1:
		...
		externalBufYAMLFile.Deps = xslices.Map(
			...
			func(moduleRef bufparse2.Ref) string {
...
func getRootToExcludes(roots []string, fullExcludes []string) (map[string][]string, error) {
	...
	for _, fullExclude := range fullExcludes {
		switch matchingRoots := normalpath2.MapAllEqualOrContainingPaths(rootMap, fullExclude, normalpath2.Relative); len(matchingRoots) {
		case 0:
		...
		case 1:
			...
			// Just in case.
			exclude, err = normalpath2.NormalizeAndValidate(exclude)
			if err != nil {
				return nil, err
			}
...
func getConfiguredDepModuleRefsForExternalDeps(
	...
	configuredDepModuleRefs := make([]bufparse2.Ref, len(externalDeps))
	for i, externalDep := range externalDeps {
		moduleRef, err := bufparse2.ParseRef(externalDep)
		if err != nil {
...
func isLintOrBreakingDisabledBasedOnIgnores(
	...
	for _, ignore := range ignores {
		ignore, err := normalpath2.NormalizeAndValidate(ignore)
		if err != nil {
			// user error
