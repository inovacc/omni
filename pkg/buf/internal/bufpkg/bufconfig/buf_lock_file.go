...
func NewBufLockFile(
	...
	policyNameToPluginKeys map[string][]bufplugin2.PluginKey,
) (BufLockFile, error) {
...
func BufLockFileWithDigestResolver(
	digestResolver func(ctx context.Context, remote string, commitID uuid.UUID) (bufmodule2.Digest, error),
) BufLockFileOption {
...
func (l *bufLockFile) DepModuleKeys() []bufmodule2.ModuleKey {
	return l.depModuleKeys
}

func (l *bufLockFile) RemotePluginKeys() []bufplugin2.PluginKey {
	return l.remotePluginKeys
}

func (l *bufLockFile) RemotePolicyKeys() []bufpolicy2.PolicyKey {
	return l.remotePolicyKeys
}

func (l *bufLockFile) PolicyNameToRemotePluginKeys() map[string][]bufplugin2.PluginKey {
		...
		for _, policy := range externalBufLockFile.Policies {
			...
			policyKey, err := bufpolicy2.NewPolicyKey(
				...
				func() (bufpolicy2.Digest, error) {
					return bufpolicy2.ParseDigest(policy.Digest)
				},
			)
			if err != nil {
				return nil, err
...
func parseRemotePluginDeps(pluginDeps []externalBufLockFileDepV2) ([]bufplugin2.PluginKey, error) {
	remotePluginKeys := make([]bufplugin2.PluginKey, len(pluginDeps))
	for i, plugin := range pluginDeps {
		...
		pluginKey, err := bufplugin2.NewPluginKey(
			...
			func() (bufplugin2.Digest, error) {
				return bufplugin2.ParseDigest(plugin.Digest)
			},
		)
		if err != nil {
			return nil, err
...
func validateModuleExpectedDigestType(
	...
	fileVersion FileVersion,
	expectedDigestType bufmodule2.DigestType,
) error {
	for _, moduleKey := range moduleKeys {
...
func validatePluginExpectedDigestType(
	pluginKeys []bufplugin2.PluginKey,
	fileVersion FileVersion,
	expectedDigestType bufplugin2.DigestType,
) error {
	for _, pluginKey := range pluginKeys {
...
func validatePolicyExpectedDigestType(
	policyKeys []bufpolicy2.PolicyKey,
	fileVersion FileVersion,
	expectedDigestType bufpolicy2.DigestType,
) error {
