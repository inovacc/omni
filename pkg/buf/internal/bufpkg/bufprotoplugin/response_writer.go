...
func (h *responseWriter) WriteResponse(
	...
	for _, file := range response.File {
		...
		} else if err := storage2.PutPath(ctx, writeBucket, file.GetName(), []byte(file.GetContent())); err != nil {
			return err
		}
	}
	return nil
...
func applyInsertionPoint(
	...
	return storage2.PutPath(ctx, writeBucket, file.GetName(), resultData)
}
...
type writeResponseOptions struct {
	insertionPointReadBucket storage2.ReadBucket
}
