...
type Client interface {
	...
	Lint(ctx context.Context, config bufconfig2.LintConfig, image bufimage.Image, options ...LintOption) error
	...
	//
	// An error of type bufanalysis.FileAnnotationSet will be returned lint failure.
	Breaking(ctx context.Context, config bufconfig2.BreakingConfig, image bufimage.Image, againstImage bufimage.Image, options ...BreakingOption) error
	// ConfiguredRules returns all of the Configured Rules for the given RuleType.
	ConfiguredRules(ctx context.Context, ruleType check.RuleType, config bufconfig2.CheckConfig, options ...ConfiguredRulesOption) ([]Rule, error)
	// AllRules returns all Rules (configured or not) for the given RuleType.
	AllRules(ctx context.Context, ruleType check.RuleType, fileVersion bufconfig2.FileVersion, options ...AllRulesOption) ([]Rule, error)
	// AllCategories returns all Categories.
	AllCategories(ctx context.Context, fileVersion bufconfig2.FileVersion, options ...AllCategoriesOption) ([]Category, error)
}
...
func WithRelatedCheckConfigs(relatedCheckConfigs ...bufconfig2.CheckConfig) LintBreakingAndConfiguredRulesOption {
...
func WithPluginConfigs(pluginConfigs ...bufconfig2.PluginConfig) ClientFunctionOption {
...
func WithPolicyConfigs(policyConfigs ...bufconfig2.PolicyConfig) ClientFunctionOption {
...
type RunnerProvider interface {
	NewRunner(plugin bufplugin2.Plugin) (pluginrpc.Runner, error)
}

// RunnerProviderFunc is a function that implements RunnerProvider.
type RunnerProviderFunc func(bufplugin2.Plugin) (pluginrpc.Runner, error)

// NewRunner implements RunnerProvider.
//
// RunnerProvider selects the correct Runner based on the type of pluginConfig.
func (r RunnerProviderFunc) NewRunner(plugin bufplugin2.Plugin) (pluginrpc.Runner, error) {
	return r(plugin)
}
...
func ClientWithRemotePolicies(
	...
	policyPluginKeyProvider bufpolicy2.PolicyPluginKeyProvider,
	policyPluginDataProvider bufpolicy2.PolicyPluginDataProvider,
) ClientOption {
	return func(clientOptions *clientOptions) {
