...
type ModuleDataStore interface {
	...
	GetModuleDatasForModuleKeys(context.Context, []bufmodule2.ModuleKey) (
		foundModuleDatas []bufmodule2.ModuleData,
		notFoundModuleKeys []bufmodule2.ModuleKey,
		err error,
	)
	...
	PutModuleDatas(ctx context.Context, moduleDatas []bufmodule2.ModuleData) error
}
...
func (p *moduleDataStore) PutModuleDatas(
	ctx context.Context,
	moduleDatas []bufmodule2.ModuleData,
) error {
...
func (p *moduleDataStore) getModuleDataForModuleKey(
	...
	if externalModuleData.V1BufYAMLFile != "" {
		...
		v1BufYAMLFileData, err := storage2.ReadPath(ctx, moduleCacheBucket, externalModuleData.V1BufYAMLFile)
		if err != nil {
			return nil, err
		}
	...
	if externalModuleData.V1BufLockFile != "" {
		...
		v1BufLockFileData, err := storage2.ReadPath(ctx, moduleCacheBucket, externalModuleData.V1BufLockFile)
		if err != nil {
			return nil, err
		}
	...
	return bufmodule2.NewModuleData(
		ctx,
		moduleKey,
		func() (storage2.ReadBucket, error) {
			return storage2.StripReadBucketExternalPaths(
				storage2.MapReadBucket(
					moduleCacheBucket,
					storage2.MapOnPrefix(externalModuleData.FilesDir),
				),
			), nil
		},
...
func (p *moduleDataStore) putModuleData(
	...
	if _, err := storage2.Copy(
		ctx,
		filesBucket,
		storage2.MapWriteBucket(moduleCacheBucket, storage2.MapOnPrefix(externalModuleDataFilesDir)),
	); err != nil {
		return err
	}
	...
	return storage2.PutPath(
		ctx,
		moduleCacheBucket,
		externalModuleDataFileName,
		data,
		storage2.PutWithAtomic(),
	)
}
...
func (p *moduleDataStore) getReadBucketForTar(
	ctx context.Context,
	moduleKey bufmodule2.ModuleKey,
) (_ storage2.ReadBucket, retErr error) {
	tarPath, err := getModuleDataStoreTarPath(moduleKey)
	if err != nil {
		return nil, err
	}
...
func (p *moduleDataStore) logDebugModuleKey(ctx context.Context, moduleKey bufmodule2.ModuleKey, message string, fields ...any) {
...
func getModuleDataStoreDirPath(moduleKey bufmodule2.ModuleKey) (string, error) {
...
func getModuleDataStoreTarPath(moduleKey bufmodule2.ModuleKey) (string, error) {
	digest, err := moduleKey.Digest()
	if err != nil {
		return "", err
	}
...
func getDepModuleKeyForExternalModuleDataDep(dep externalModuleDataDep) (bufmodule2.ModuleKey, error) {
	if dep.Name == "" {
...
func getModuleDataStoreDirLockPath(moduleKey bufmodule2.ModuleKey) (string, error) {
