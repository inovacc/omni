...
type CommitStore interface {
	...
	GetCommitsForModuleKeys(ctx context.Context, moduleKeys []bufmodule2.ModuleKey) (
		foundCommits []bufmodule2.Commit,
	...
	// Returns the found Commits, and the input IDs that were not found, each
	// ordered by the order of the input IDs.
	GetCommitsForCommitKeys(ctx context.Context, commitKeys []bufmodule2.CommitKey) (
		foundCommits []bufmodule2.Commit,
		notFoundCommitKeys []bufmodule2.CommitKey,
		err error,
	)
	// PutCommits puts the Commits to the store.
	PutCommits(ctx context.Context, commits []bufmodule2.Commit) error
}
...
func (p *commitStore) GetCommitsForModuleKeys(
...
) ([]bufmodule2.Commit, []bufmodule2.ModuleKey, error) {
	var foundCommits []bufmodule2.Commit
	...
	for _, moduleKey := range moduleKeys {
		...
		commitKey, err := bufmodule2.ModuleKeyToCommitKey(moduleKey)
		if err != nil {
			return nil, nil, err
		}
...
) ([]bufmodule2.Commit, []bufmodule2.CommitKey, error) {
	var foundCommits []bufmodule2.Commit
	var notFoundCommitKeys []bufmodule2.CommitKey
...
func (p *commitStore) getCommitForCommitKey(
	ctx context.Context,
	commitKey bufmodule2.CommitKey,
	// may be nil
	expectedDigest bufmodule2.Digest,
) (_ bufmodule2.Commit, retErr error) {
	...
	digest, err := bufmodule2.ParseDigest(externalCommit.Digest)
	if err != nil {
...
func (p *commitStore) putCommit(
	...
	return storage2.PutPath(ctx, bucket, path, data, storage2.PutWithAtomic())
}

func (p *commitStore) getReadWriteBucketForDir(ctx context.Context, commitKey bufmodule2.CommitKey) storage2.ReadWriteBucket {
	...
	return storage2.MapReadWriteBucket(p.bucket, storage2.MapOnPrefix(dirPath))
}

func (p *commitStore) deleteInvalidCommitFile(
	ctx context.Context,
	commitKey bufmodule2.CommitKey,
	bucket storage2.WriteBucket,
	path string,
...
func (p *commitStore) logDebugCommitKey(ctx context.Context, commitKey bufmodule2.CommitKey, message string, fields ...any) {
...
func getCommitStoreFilePath(commitKey bufmodule2.CommitKey) string {
