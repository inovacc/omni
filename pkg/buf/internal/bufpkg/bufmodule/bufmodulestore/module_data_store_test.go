...
func testModuleDataStore(
	...
	if tar {
		...
		require.NoError(t, err)
		require.NoError(t, storage2.PutPath(ctx, bucket, tarPath, []byte("invalid_tar")))
	} else {
...
func testGetModuleKeysAndModuleDatas(t *testing.T, ctx context.Context) ([]bufmodule2.ModuleKey, []bufmodule2.ModuleData) {
...
func testRequireModuleKeyNamesEqual(t *testing.T, expected []string, actual []bufmodule2.ModuleKey) {
	if len(expected) == 0 {
		...
		require.Equal(
			...
			xslices.Map(
				actual,
				func(value bufmodule2.ModuleKey) string {
...
func testRequireModuleDataNamesEqual(t *testing.T, expected []string, actual []bufmodule2.ModuleData) {
	if len(expected) == 0 {
		require.Equal(t, 0, len(actual))
	} else {
		require.Equal(
			...
			xslices.Map(
				actual,
				func(value bufmodule2.ModuleData) string {
