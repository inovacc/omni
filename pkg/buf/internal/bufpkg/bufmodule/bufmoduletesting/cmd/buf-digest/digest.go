...
func (f *flags) Bind(flagSet *pflag.FlagSet) {
	flagSet.StringVar(
		&f.DigestType,
		digestTypeFlagName,
		bufmodule2.DigestTypeB5.String(),
		fmt.Sprintf(
			"The digest type. Must be one of %s",
			xstrings.SliceToString(xslices.Map(bufmodule2.AllDigestTypes, bufmodule2.DigestType.String)),
		),
	)
}
...
func run(
	...
	digestType, err := bufmodule2.ParseDigestType(flags.DigestType)
	if err != nil {
	...
	moduleSetBuilder := bufmodule2.NewModuleSetBuilder(ctx, container.Logger(), bufmodule2.NopModuleDataProvider, bufmodule2.NopCommitProvider)
