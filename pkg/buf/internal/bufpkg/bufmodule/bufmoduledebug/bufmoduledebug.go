...
import (
	...
	"github.com/inovacc/omni/pkg/buf/internal/pkg/indent"
)
...
func ModuleSetDebugString(ctx context.Context, moduleSet bufmodule2.ModuleSet) (string, error) {
...
//
// This can be printed to stderr, debug logged, etc, while debugging.
func ModuleDebugString(ctx context.Context, module bufmodule2.Module) (string, error) {
	printer := indent.NewPrinter("  ")
	if err := printModule(ctx, printer, module); err != nil {
		return "", err
	}
...
//
// This can be printed to stderr, debug logged, etc, while debugging.
func ModuleReadBucketDebugString(ctx context.Context, moduleReadBucket bufmodule2.ModuleReadBucket) (string, error) {
	printer := indent.NewPrinter("  ")
...
func printModule(ctx context.Context, printer indent.Printer, module bufmodule2.Module) error {
