...
func TestBasic(t *testing.T) {
	...
	// local sources.
	moduleSetBuilder := bufmodule2.NewModuleSetBuilder(ctx, slogtestext.NewLogger(t), bsrProvider, bsrProvider)
	...
	moduleRefExtdep1, err := bufparse2.NewRef("buf.build", "foo", "extdep1", "")
	require.NoError(t, err)
	moduleRefExtdep2, err := bufparse2.NewRef("buf.build", "foo", "extdep2", "")
	require.NoError(t, err)
	moduleRefExtdep3, err := bufparse2.NewRef("buf.build", "foo", "extdep3", "")
	require.NoError(t, err)
	moduleRefExtdep4, err := bufparse2.NewRef("buf.build", "foo", "extdep4", "")
	require.NoError(t, err)
	moduleRefModule2, err := bufparse2.NewRef("buf.build", "bar", "module2", "")
	require.NoError(t, err)
	...
	moduleFullName, err := bufparse2.NewFullName("buf.build", "bar", "module2")
	require.NoError(t, err)
	moduleSetBuilder.AddLocalModule(
		testNewBucketForPathToData(
			t,
		...
		"path/to/module2",
		true,
		bufmodule2.LocalModuleWithFullName(moduleFullName),
		...
		bufmodule2.LocalModuleWithTargetPaths(nil, []string{"foo"}),
	)
	...
	require.Equal(
		...
		bufmodule2.ModuleSetOpaqueIDs(moduleSet),
	)
	require.Equal(
		t,
		[]string{
		...
		bufmodule2.ModuleSetTargetOpaqueIDs(moduleSet),
	)
	...
	// that we expect.
	graph, err := bufmodule2.ModuleSetToDAG(moduleSet)
	require.NoError(t, err)
	...
	require.Equal(
		...
		xslices.Map(transitiveRemoteDeps, func(remoteDep bufmodule2.RemoteDep) string { return remoteDep.OpaqueID() }),
	)
}
...
func TestDuplicateProtoPathError(t *testing.T) {
	...
	checkError(err)
	err = moduleReadBucket.WalkFileInfos(ctx, func(bufmodule2.FileInfo) error { return nil })
	checkError(err)
}
...
func testFilePaths(t *testing.T, module bufmodule2.Module, expectedFilePaths ...string) {
	ctx := context.Background()
	fileInfos, err := bufmodule2.GetFileInfos(ctx, module)
	require.NoError(t, err)
	require.Equal(
		t,
		expectedFilePaths,
		bufmodule2.FileInfoPaths(fileInfos),
	)
}

func testTargetFilePaths(t *testing.T, module bufmodule2.Module, expectedFilePaths ...string) {
	ctx := context.Background()
	fileInfos, err := bufmodule2.GetTargetFileInfos(ctx, module)
	require.NoError(t, err)
	require.Equal(
		t,
		expectedFilePaths,
