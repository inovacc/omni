...
func NewDigest(digestType DigestType, bufcasDigest bufcas2.Digest) (Digest, error) {
	switch digestType {
	case DigestTypeB4, DigestTypeB5:
		if bufcasDigest.Type() != bufcas2.DigestTypeShake256 {
...
func newDigest(digestType DigestType, bufcasDigest bufcas2.Digest) *digest {
	return &digest{
		digestType:   digestType,
...
func getB4Digest(
	...
	if err := storage2.WalkReadObjects(
		...
		storage2.FilterReadBucket(bucketWithStorageMatcherApplied, getStorageMatcher(ctx, bucketWithStorageMatcherApplied)),
		"",
		func(readObject storage2.ReadObject) error {
			digest, err := bufcas2.NewDigestForContent(readObject)
			if err != nil {
				return err
			}
			fileNode, err := bufcas2.NewFileNode(readObject.Path(), digest)
			if err != nil {
				return err
			}
...
func getB5DigestForBucketAndDepDigests(
	...
	digestOfDigests, err := bufcas2.NewDigestForContent(strings.NewReader(strings.Join(digestStrings, "\n")))
	if err != nil {
		return nil, err
	}
