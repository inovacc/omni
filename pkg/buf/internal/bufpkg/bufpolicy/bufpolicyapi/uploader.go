...
func (u *uploader) Upload(
	ctx context.Context,
	policies []bufpolicy2.Policy,
	options ...bufpolicy2.UploadOption,
) ([]bufpolicy2.Commit, error) {
	uploadOptions, err := bufpolicy2.NewUploadOptions(options)
	if err != nil {
		return nil, err
	}
...
func (u *uploader) uploadIndexedPoliciesForRegistry(
	ctx context.Context,
	registry string,
	indexedPolicies []xslices.Indexed[bufpolicy2.Policy],
	uploadOptions bufpolicy2.UploadOptions,
) ([]xslices.Indexed[bufpolicy2.Commit], error) {
	...
	for i, policyCommit := range policyCommits {
		...
		indexedCommits = append(
			indexedCommits,
			xslices.Indexed[bufpolicy2.Commit]{
				Value: commit,
				Index: i,
			},
		)
...
func (u *uploader) createPolicyIfNotExist(
	...
	policy bufpolicy2.Policy,
	createPolicyVisibility bufpolicy2.PolicyVisibility,
	...
	if err != nil {
		if connect.CodeOf(err) == connect.CodeAlreadyExists {
			...
			policies, err := u.validatePoliciesExist(ctx, primaryRegistry, []bufpolicy2.Policy{policy})
			if err != nil {
				return nil, err
			}
			if len(policies) != 1 {
		...
		connect.NewRequest(
			&policyv1beta1.GetPoliciesRequest{
				PolicyRefs: xslices.Map(
					policies,
					func(policy bufpolicy2.Policy) *policyv1beta1.PolicyRef {
