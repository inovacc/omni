...
func (p *policyDataProvider) GetPolicyDatasForPolicyKeys(
	...
	registryToIndexedPolicyKeys := xslices.ToIndexedValuesMap(
		policyKeys,
		func(policyKey bufpolicy2.PolicyKey) string {
...
func (p *policyDataProvider) getIndexedPolicyDatasForRegistryAndIndexedPolicyKeys(
	ctx context.Context,
	registry string,
	indexedPolicyKeys []xslices.Indexed[bufpolicy2.PolicyKey],
) ([]xslices.Indexed[bufpolicy2.PolicyData], error) {
	values := xslices.Map(indexedPolicyKeys, func(indexedPolicyKey xslices.Indexed[bufpolicy2.PolicyKey]) *policyv1beta1.DownloadRequest_Value {
	...
	commitIDToIndexedPolicyKeys, err := xslices.ToUniqueValuesMapError(
		indexedPolicyKeys,
		func(indexedPolicyKey xslices.Indexed[bufpolicy2.PolicyKey]) (uuid.UUID, error) {
