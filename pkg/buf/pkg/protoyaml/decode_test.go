...
func TestExtension(t *testing.T) {
	...
	actual := &testv2.Proto2Test{}
	err := Unmarshal([]byte(`[buf.protoyaml.test.v1.p2t_string_ext]: hi`), actual)
	require.NoError(t, err)
	require.Equal(t, "hi", proto.GetExtension(actual, testv2.E_P2TStringExt))
}

func TestEditions(t *testing.T) {
	t.Parallel()

	expected := &testv2.EditionsTest{
		Name: proto.String("foobar"),
		Nested: &testv2.EditionsTest_Nested{
			Ids: []int64{0, 1, 1, 2, 3, 5, 8},
		},
		Enum: testv2.OpenEnum_OPEN_ENUM_UNSPECIFIED,
	}
	actual := &testv2.EditionsTest{}
	data := []byte(`
name: "foobar"
