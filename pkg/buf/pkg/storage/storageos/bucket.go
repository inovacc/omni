...
func newBucket(rootPath string, symlinks bool) (*bucket, error) {
	rootPath = normalpath2.Unnormalize(rootPath)
...
func (b *bucket) Get(ctx context.Context, path string) (storage2.ReadObjectCloser, error) {
...
func (b *bucket) Stat(ctx context.Context, path string) (storage2.ObjectInfo, error) {
	externalPath, err := b.getExternalPath(path)
	if err != nil {
		return nil, err
	}
...
func (b *bucket) Walk(
	ctx context.Context,
	prefix string,
	f func(storage2.ObjectInfo) error,
) error {
	...
	if err := xfilepath.Walk(
		...
		func(externalPath string, fileInfo os.FileInfo, err error) error {
			...
			if fileInfo.Mode().IsRegular() {
				path, err := normalpath2.Rel(b.absoluteRootPath, absoluteExternalPath)
				if err != nil {
					return err
				}
				// just in case
				path, err = normalpath2.NormalizeAndValidate(path)
				if err != nil {
					return err
				}
				if err := f(
...
func (b *bucket) Put(ctx context.Context, path string, options ...storage2.PutOption) (storage2.WriteObjectCloser, error) {
	externalPath, err := b.getExternalPath(path)
	if err != nil {
		return nil, err
	}
...
func (w *writeObjectCloser) SetExternalPath(string) error {
	return storage2.ErrSetExternalPathUnsupported
}

func (w *writeObjectCloser) SetLocalPath(string) error {
	return storage2.ErrSetLocalPathUnsupported
}
...
func newErrNotDir(path string) *normalpath2.Error {
	return normalpath2.NewError(path, errNotDir)
}
