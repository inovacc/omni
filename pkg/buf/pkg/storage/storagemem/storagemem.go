...
import (
	...
	"github.com/inovacc/omni/pkg/buf/pkg/storage/storagemem/internal"
...
func NewReadWriteBucket() storage2.ReadWriteBucket {
	return newBucket(nil)
}

// NewReadBucket returns a new ReadBucket.
func NewReadBucket(pathToData map[string][]byte) (storage2.ReadBucket, error) {
...
func CopyReadBucket(ctx context.Context, inputBucket storage2.ReadBucket) (storage2.ReadBucket, error) {
	if _, ok := inputBucket.(*bucket); ok {
		return inputBucket, nil
	}
	inMemoryBucket := newBucket(nil)
	if _, err := storage2.Copy(ctx, inputBucket, inMemoryBucket, storage2.CopyWithExternalAndLocalPaths()); err != nil {
		return nil, err
	}
