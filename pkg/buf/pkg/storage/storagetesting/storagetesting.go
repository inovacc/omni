...
import (
	...
	"github.com/inovacc/omni/pkg/buf/pkg/storage/storageutil"
	"github.com/inovacc/omni/pkg/buf/pkg/tmp"
...
func AssertNotExist(
	t *testing.T,
	readBucket storage2.ReadBucket,
	path string,
) {
...
func AssertObjectInfoEqual(
	t *testing.T,
	expected storage2.ObjectInfo,
	actual storage2.ObjectInfo,
) {
...
func AssertPathToContent(
	...
	require.NoError(t, readBucket.Walk(
		context.Background(),
		walkPrefix,
		func(objectInfo storage2.ObjectInfo) error {
...
func RunTestSuite(
	...
	newReadBucket func(*testing.T, string, storageos.Provider) (storage2.ReadBucket, GetExternalPathFunc),
	newWriteBucket func(*testing.T, storageos.Provider) storage2.WriteBucket,
	writeBucketToReadBucket func(*testing.T, storage2.WriteBucket) storage2.ReadBucket,
	...
	t.Run("map-1", func(t *testing.T) {
		t.Parallel()
		readBucket, _ := newReadBucket(t, oneDirPath, defaultProvider)
		readBucket = storage2.MapReadBucket(
			readBucket,
			storage2.MapOnPrefix("root"),
		)
		AssertPathToContent(
			t,
			readBucket,
	...
	t.Run("map-3", func(t *testing.T) {
		...
		readBucket = storage2.FilterReadBucket(
			readBucket,
			storage2.MatchOr(
				storage2.MatchPathExt(".txt"),
				storage2.MatchPathEqual("2.proto"),
			),
		)
		AssertPathToContent(
			t,
	...
	t.Run("multi-all", func(t *testing.T) {
		...
		readBucketMulti := storage2.MultiReadBucket(
			storage2.MapReadBucket(
				readBucket,
				storage2.MapOnPrefix("root1"),
			),
			storage2.MapReadBucket(
				readBucket,
				storage2.MapOnPrefix("root2"),
			),
		)
		AssertPathToContent(
			t,
	...
	t.Run("multi-overlapping-files-error", func(t *testing.T) {
		...
		readBucketMulti := storage2.MultiReadBucket(
			...
			storage2.MapReadBucket(
				readBucket,
				storage2.MapOnPrefix("rootoverlap"),
			),
		)
		...
		assert.Error(t, err)
		assert.True(t, storage2.IsExistsMultipleLocations(err))
		...
		err = readBucketMulti.Walk(
			context.Background(),
			"",
			func(storage2.ObjectInfo) error {
				return nil
			},
		)
		assert.Error(t, err)
	...
	for _, testCase := range []struct {
		...
		newReadBucketFunc     func(*testing.T) storage2.ReadBucket
		mappers               []storage2.Mapper
		matchers              []storage2.Matcher
		expectedPathToContent map[string]string
	}{
		{
			name: "proto-and-single-file",
			newReadBucketFunc: func(t *testing.T) storage2.ReadBucket {
				readBucket, _ := newReadBucket(t, oneDirPath, defaultProvider)
				return readBucket
			},
			matchers: []storage2.Matcher{
				storage2.MatchOr(
					storage2.MatchPathExt(".proto"),
					storage2.MatchPathEqual("root/foo.yaml"),
				),
			},
			expectedPathToContent: map[string]string{
				"root/a/b/1.proto": testProtoContent,
		...
		},
		{
			...
			matchers: []storage2.Matcher{
				storage2.MatchOr(
					storage2.MatchPathExt(".proto"),
					storage2.MatchPathEqual("foo.yaml"),
				),
			},
			expectedPathToContent: map[string]string{
				"root/a/b/1.proto": testProtoContent,
		...
		},
		{
			...
			matchers: []storage2.Matcher{
				storage2.MatchOr(
					storage2.MatchPathExt(".proto"),
					storage2.MatchPathEqual("a/bar.yaml"),
				),
			},
			expectedPathToContent: map[string]string{
				"a/b/1.proto": testProtoContent,
		...
		},
		{
			...
			matchers: []storage2.Matcher{
				storage2.MatchOr(
					storage2.MatchPathExt(".proto"),
					storage2.MatchPathEqual("bar.yaml"),
				),
			},
			expectedPathToContent: map[string]string{
				"b/1.proto": testProtoContent,
		...
		},
		{
			...
			matchers: []storage2.Matcher{
				storage2.MatchNot(
					storage2.MatchPathContained("a"),
				),
				storage2.MatchOr(
					storage2.MatchPathExt(".proto"),
					storage2.MatchPathEqualOrContained("foo.yaml"),
				),
			},
			expectedPathToContent: map[string]string{
				"ab/1.proto": testProtoContent,
		...
		},
		{
			...
			matchers: []storage2.Matcher{
				storage2.MatchAnd(
					...
					storage2.MatchNot(
						storage2.MatchOr(
							storage2.MatchPathEqualOrContained("a"),
							storage2.MatchPathEqualOrContained("c"),
						),
					),
				),
			},
		...
		},
		{
			...
			newReadBucketFunc: func(t *testing.T) storage2.ReadBucket {
				readBucket, _ := newReadBucket(t, twoDirPath, defaultProvider)
				return storage2.MultiReadBucket(
					storage2.MapReadBucket(
						readBucket,
						storage2.MapOnPrefix("root1"),
					),
		...
		if testCase.stripComponentCount == 0 {
			t.Run(fmt.Sprintf("copy-%s", testCase.name), func(t *testing.T) {
				...
				readBucket = storage2.MapReadBucket(readBucket, testCase.mappers...)
				readBucket = storage2.FilterReadBucket(readBucket, testCase.matchers...)
	...
	t.Run("diff", func(t *testing.T) {
		...
		readBucketA = storage2.MapReadBucket(readBucketA, storage2.MapOnPrefix("prefix"))
		readBucketB = storage2.MapReadBucket(readBucketB, storage2.MapOnPrefix("prefix"))
		...
		diff, err := storage2.DiffBytes(
			context.Background(),
			readBucketA,
			readBucketB,
			storage2.DiffWithSuppressTimestamps(),
			storage2.DiffWithExternalPaths(),
			storage2.DiffWithExternalPathPrefixes(
				externalPathPrefixA,
				externalPathPrefixB,
			),
		)
...
%s`,
	...
	t.Run("overlap-success", func(t *testing.T) {
		...
		readBucket = storage2.FilterReadBucket(readBucket, storage2.MatchPathExt(".proto"))
		readBucket = storage2.MultiReadBucket(
			storage2.MapReadBucket(
				readBucket,
				storage2.MapOnPrefix("a"),
			),
		...
		allPaths, err := storage2.AllPaths(
			context.Background(),
			readBucket,
			"",
		)
	...
	t.Run("map-write-bucket", func(t *testing.T) {
		t.Parallel()
		writeBucket := newWriteBucket(t, defaultProvider)
		mapWriteBucket := storage2.MapWriteBucket(
			writeBucket,
			storage2.MapOnPrefix("a/b/c"),
		)
	...
	t.Run("walk-prefixed-bucket-should-not-error", func(t *testing.T) {
		...
		writeBucket := newWriteBucket(t, defaultProvider)
		readBucket := writeBucketToReadBucket(t, writeBucket)
		mappedReadBucket := storage2.MapReadBucket(readBucket, storage2.MapOnPrefix("prefix"))
		err := mappedReadBucket.Walk(context.Background(), "", func(_ storage2.ObjectInfo) error {
			return nil
		})
		require.NoError(t, err)
	})
	...
	t.Run("is_empty", func(t *testing.T) {
		...
		isEmpty, err = storage2.IsEmpty(ctx, readBucket, "root/a")
		require.NoError(t, err)
		require.False(t, isEmpty)
		...
		require.NoError(t, err)
		require.False(t, isEmpty)
		isEmpty, err = storage2.IsEmpty(
			ctx,
			storage2.FilterReadBucket(readBucket, storage2.MatchPathExt(".proto")),
			"",
		)
		require.NoError(t, err)
		require.True(t, isEmpty)
	...
	t.Run("overlay", func(t *testing.T) {
		...
		readBucket := storage2.OverlayReadBucket(aReadBucket, bReadBucket)
		AssertPathToContent(
			t,
			readBucket,
			"",
