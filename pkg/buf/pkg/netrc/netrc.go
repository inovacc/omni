...
func GetMachineForName(envContainer app2.EnvContainer, name string) (_ Machine, retErr error) {
...
func PutMachines(envContainer app2.EnvContainer, machines ...Machine) error {
	filePath, err := GetFilePath(envContainer)
	if err != nil {
		return err
	}
...
func DeleteMachineForName(envContainer app2.EnvContainer, name string) (bool, error) {
	filePath, err := GetFilePath(envContainer)
	if err != nil {
		return false, err
	}
...
func GetFilePath(envContainer app2.EnvContainer) (string, error) {
	if netrcFilePath := envContainer.Env("NETRC"); netrcFilePath != "" {
		return netrcFilePath, nil
	}
	homeDirPath, err := app2.HomeDirPath(envContainer)
	if err != nil {
		return "", err
	}
