...
func Parallelize(ctx context.Context, jobs []func(context.Context) error, options ...ParallelizeOption) error {
	...
	if parallelism < 1 {
		...
		parallelism = Parallelism() * multiplier
	}
...
// WithParallelism returns a new ParallelizeOption that sets the absolute
// number of goroutines that can run simultaneously.
//
// Values less than 1 are ignored and the default parallelism is used.
// This takes precedence over ParallelizeWithMultiplier if both are set.
