...
func LoggerProvider(container appext2.NameContainer, logLevel appext2.LogLevel, logFormat appext2.LogFormat) (*slog.Logger, error) {
	return NewLogger(container.Stderr(), logLevel, logFormat)
}

// NewLogger returns a new Logger for the given level and format.
func NewLogger(writer io.Writer, logLevel appext2.LogLevel, logFormat appext2.LogFormat) (*slog.Logger, error) {
...
func getLevel(logLevel appext2.LogLevel) (slog.Level, error) {
	switch logLevel {
	case appext2.LogLevelDebug:
		return slog.LevelDebug, nil
	case appext2.LogLevelInfo:
		return slog.LevelInfo, nil
	case appext2.LogLevelWarn:
		return slog.LevelWarn, nil
	case appext2.LogLevelError:
...
func getHandler(writer io.Writer, logLevel appext2.LogLevel, logFormat appext2.LogFormat) (slog.Handler, error) {
	level, err := getLevel(logLevel)
	if err != nil {
		return nil, err
	}
	switch logFormat {
	case appext2.LogFormatJSON:
		return slog.NewJSONHandler(writer, &slog.HandlerOptions{
			Level:       level,
			ReplaceAttr: defaultReplaceAttr,
		}), nil
	case appext2.LogFormatText:
