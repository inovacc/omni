...
import (
	...
	"github.com/inovacc/omni/pkg/buf/cmd/buf/xstrings/xlog/xslog"
...
func run(
	...
	if env.PrintFreeFieldNumbers {
		fileInfos, err := bufmodule2.GetTargetFileInfos(
			ctx,
	...
	return controller.PutImage(
		...
		bufctl2.WithImageExcludeSourceInfo(!env.IncludeSourceInfo),
		bufctl2.WithImageExcludeImports(!env.IncludeImports),
		bufctl2.WithImageAsFileDescriptorSet(true),
	)
}
