...
func run(
	...
	bufcli2.WarnAlphaCommand(ctx, container)
	...
	if !flags.Force {
		if err := bufcli2.PromptUserForDelete(container, "token", flags.TokenID); err != nil {
			return err
		}
	}
	if _, err := service.DeleteToken(
		...
		if connect.CodeOf(err) == connect.CodeNotFound {
			return bufcli2.NewTokenNotFoundError(flags.TokenID)
		}
		return err
	}
	return nil
