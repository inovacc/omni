...
func ModuleKeysAndTransitiveDepModuleKeysForModuleRefs(
	...
	moduleKeyProvider, err := bufcli2.NewModuleKeyProvider(container)
	if err != nil {
		return nil, err
	}
...
func Prune(
	...
	bufYAMLBasedDepModuleKeys []bufmodule2.ModuleKey,
	workspaceDepManager bufworkspace2.WorkspaceDepManager,
	dirPath string,
) error {
	workspace, err := controller.GetWorkspace(ctx, dirPath, bufctl2.WithIgnoreAndDisallowV1BufWorkYAMLs())
	if err != nil {
		return err
	}
	...
	if _, err := controller.GetImageForWorkspace(
		ctx,
		workspace,
		bufctl2.WithImageExcludeSourceInfo(true),
	); err != nil {
		return err
	}
	...
	depModules, err := bufmodule2.RemoteDepsForModuleSet(workspace)
	if err != nil {
		return err
	}
	depModuleKeys, err := xslices.MapError(
		depModules,
		func(remoteDep bufmodule2.RemoteDep) (bufmodule2.ModuleKey, error) {
			return bufmodule2.ModuleToModuleKey(remoteDep, workspaceDepManager.BufLockFileDigestType())
		},
	)
	if err != nil {
		return err
...
func LogUnusedConfiguredDepsForWorkspace(
	...
	malformedDeps, err := bufworkspace2.MalformedDepsForWorkspace(workspace)
	if err != nil {
		return err
	}
	for _, malformedDep := range malformedDeps {
		switch t := malformedDep.Type(); t {
		case bufworkspace2.MalformedDepTypeUnused:
...
func moduleKeysAndTransitiveDepModuleKeysForModuleKeys(
	...
	graphProvider, err := bufcli2.NewGraphProvider(container)
	if err != nil {
		return nil, err
	}
	...
	var newModuleKeys []bufmodule2.ModuleKey
	if err := graph.WalkNodes(
		func(moduleKey bufmodule2.ModuleKey, _ []bufmodule2.ModuleKey, _ []bufmodule2.ModuleKey) error {
...
func validateModuleKeysContains(containingModuleKeys []bufmodule2.ModuleKey, moduleKeys []bufmodule2.ModuleKey) error {
...
func getFullNameStringToModuleKey(moduleKeys []bufmodule2.ModuleKey) (map[string]bufmodule2.ModuleKey, error) {
