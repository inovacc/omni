...
` + bufcli2.GetSourceOrModuleLong(`the source or module to print the dependency graph for`),
...
func run(
	...
	case jsonFormatString:
		...
		if err := graph.WalkNodes(
			func(module bufmodule2.Module, _ []bufmodule2.Module, deps []bufmodule2.Module) error {
...
func moduleToString(module bufmodule2.Module) string {
...
func moduleFullNameOrOpaqueID(module bufmodule2.Module) string {
	if moduleFullName := module.FullName(); moduleFullName != nil {
		return moduleFullName.String()
	}
	return module.OpaqueID()
...
func dashlessCommitIDStringForModule(module bufmodule2.Module) string {
...
func (e *externalModule) addDeps(
	deps []bufmodule2.Module,
	graph *dag.Graph[string, bufmodule2.Module],
...
func externalModuleNoDepsForModule(module bufmodule2.Module) (externalModule, error) {
	// We always calculate the b5 digest here, we do not check the digest type that is stored
	// in buf.lock.
	digest, err := module.Digest(bufmodule2.DigestTypeB5)
	if err != nil {
