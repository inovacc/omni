...
func getExternalModules(
	...
	if configOverride != "" {
		bufYAMLFile, err := bufconfig2.GetBufYAMLFileForOverride(configOverride)
		if err != nil {
			return nil, err
		}
	...
	bufWorkYAMLFile, err := bufcli2.GetBufWorkYAMLFileForDirPath(ctx, ".")
	if err != nil {
		if !errors.Is(err, fs.ErrNotExist) {
			return nil, err
		}
		// We do not have a buf.work.yaml file, attempt to read a buf.yaml file.
		bufYAMLFile, err := bufcli2.GetBufYAMLFileForDirPath(ctx, ".")
		if err != nil {
			if !errors.Is(err, fs.ErrNotExist) {
				return nil, err
			}
	...
	_, err = bufcli2.GetBufYAMLFileForDirPath(ctx, ".")
...
func getExternalModulesForBufWorkYAMLFile(
	...
	for _, dirPath := range bufWorkYAMLFile.DirPaths() {
		bufYAMLFile, err := bufcli2.GetBufYAMLFileForDirPath(ctx, dirPath)
		if err != nil {
			if !errors.Is(err, fs.ErrNotExist) {
				return nil, err
			}
