...
func (f *flags) Bind(flagSet *pflag.FlagSet) {
	...
	flagSet.StringVar(
		...
		fmt.Sprintf(
			...
			xstrings.SliceToString(bufcli2.AllRuleFormatStrings),
		),
	)
	flagSet.StringVar(
		&f.Version,
		...
		fmt.Sprintf(
			...
			configuredOnlyFlagName,
			xslices.Map(
				bufconfig2.AllFileVersions,
				func(fileVersion bufconfig2.FileVersion) string {
...
func lsRun(
	...
	bufYAMLFile, err := bufcli2.GetBufYAMLFileForDirPathOrOverride(ctx, ".", configOverride)
	if err != nil {
		if !errors.Is(err, fs.ErrNotExist) {
			return err
		}
		bufYAMLFile, err = bufconfig2.NewBufYAMLFile(
			bufconfig2.FileVersionV2,
			[]bufconfig2.ModuleConfig{
				bufconfig2.DefaultModuleConfigV2,
			},
			nil,
			nil,
			nil,
	...
	if flags.ConfiguredOnly {
		...
		var moduleConfig bufconfig2.ModuleConfig
		...
		var checkConfig bufconfig2.CheckConfig
		...
		allCheckConfigs := make([]bufconfig2.CheckConfig, 0, len(moduleConfigs)*2)
...
func getModuleConfigForModulePath(moduleConfigs []bufconfig2.ModuleConfig, modulePath string) (bufconfig2.ModuleConfig, error) {
