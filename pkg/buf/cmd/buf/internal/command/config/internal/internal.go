...
func lsRun(
	...
	if flags.ConfiguredOnly {
		if flags.Version != "" {
			return appcmd2.NewInvalidArgumentErrorf("--%s cannot be specified if --%s is specified", versionFlagName, configFlagName)
		}
	} else {
		if flags.Config != "" {
			return appcmd2.NewInvalidArgumentErrorf("--%s must be set if --%s is specified", configuredOnlyFlagName, configFlagName)
		}
		if flags.ModulePath != "" {
			return appcmd2.NewInvalidArgumentErrorf("--%s must be set if --%s is specified", configuredOnlyFlagName, modulePathFlagName)
		}
	}
	...
	if flags.ConfiguredOnly {
		...
		case bufconfig.FileVersionV2:
			...
			default:
				if flags.ModulePath == "" {
					return appcmd2.NewInvalidArgumentErrorf("--%s must be specified if the the buf.yaml has more than one module", modulePathFlagName)
				}
