...
func testCompareConfigMigrate(t *testing.T, dir string, expectCode int, expectStderr string) {
	...
	_, err = storage2.Copy(ctx, inputBucket, tempBucket)
	require.NoError(t, err)
	var outputBucket storage2.ReadWriteBucket
	if expectCode == 0 {
	...
	require.NoError(t, storage2.Diff(ctx, &diff, outputBucket, tempBucket))
