...
func run(
	...
	if err := bufcli2.ValidateRequiredFlag(outDirPathFlagName, flags.OutDirPath); err != nil {
		return err
	}
	exists, err := bufcli2.BufYAMLFileExistsForDirPath(ctx, flags.OutDirPath)
	if err != nil {
		return err
	}
	if exists {
	...
	bufYAMLFile, err := bufconfig2.NewBufYAMLFile(
		...
		nil,
		nil,
		bufconfig2.BufYAMLFileWithIncludeDocsLink(),
	)
	if err != nil {
		return err
	}

	return bufcli2.PutBufYAMLFileForDirPath(ctx, flags.OutDirPath, bufYAMLFile)
}
