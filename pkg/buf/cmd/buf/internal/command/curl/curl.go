...
import (
	...
	bufcurl2 "github.com/inovacc/omni/pkg/buf/internal/buf/bufcurl"
...
func (f *flags) validate(hasURL, isSecure bool) error {
	...
	if f.Reflect {
		...
		if _, err := bufcurl2.ParseReflectProtocol(f.ReflectProtocol); err != nil {
			return fmt.Errorf(
				"--%s value must be one of %s",
				reflectProtocolFlagName,
				xstrings.SliceToHumanStringOrQuoted(bufcurl2.AllKnownReflectProtocolStrings),
			)
		}
	}
...
func (f *flags) getTLSConfig(authority string, printer verbose.Printer) (*tls.Config, error) {
	return bufcurl2.MakeVerboseTLSConfig(&bufcurl2.TLSSettings{
		KeyFile:             f.Key,
		CertFile:            f.Cert,
		CACertFile:          f.CACert,
...
func promptForPassword(ctx context.Context, container app.Container, prompt string) (string, error) {
	...
	go func() {
		defer close(ch)
		password, err = bufcli2.PromptUserForPassword(container, prompt)
	}()
	select {
	case <-ch:
		return password, err
...
func run(ctx context.Context, container appext.Container, f *flags) (err error) {
	...
	if f.Protocol != connect.ProtocolGRPC {
		...
		// is drained.
		clientOptions = append(clientOptions, connect.WithInterceptors(bufcurl2.TraceTrailersInterceptor(verbosePrinter)))
	}
	...
	requestHeaders, dataReader, err := bufcurl2.LoadHeaders(f.Headers, dataFileReference, nil)
	if err != nil {
		return err
	}
	userAgent := f.UserAgent
	if userAgent == "" {
		userAgent = bufcurl2.DefaultUserAgent(f.Protocol, bufcli2.Version)
	}
	...
	if dataReader == nil {
		if dataFileReference == "-" {
			...
			if err != nil {
				return bufcurl2.ErrorHasFilename(err, dataFileReference)
			}
			dataReader = f
		} else if f.Data != "" {
	...
	makeTransportOnce := sync.OnceValues(func() (connect.HTTPClient, error) {
		...
		roundTripper, err := makeHTTPRoundTripper(f, isSecure, bufcurl2.GetAuthority(host, requestHeaders), verbosePrinter)
		if err != nil {
			return nil, err
		}
		return bufcurl2.NewVerboseHTTPClient(roundTripper, verbosePrinter), nil
	})
	...
	if f.Output != "" {
		...
		if err != nil {
			return bufcurl2.ErrorHasFilename(err, f.Output)
		}
	}

	resolvers := make([]bufcurl2.Resolver, 0, len(f.Schemas)+2)
	if f.Reflect {
		reflectHeaders, _, err := bufcurl2.LoadHeaders(f.ReflectHeaders, "", requestHeaders)
		if err != nil {
			return err
		}
		...
		reflectProtocol, err := bufcurl2.ParseReflectProtocol(f.ReflectProtocol)
		if err != nil {
			return err
		}
		...
		res, closeRes := bufcurl2.NewServerReflectionResolver(ctx, transport, clientOptions, baseURL, reflectProtocol, reflectHeaders, verbosePrinter)
		defer closeRes()
		resolvers = append(resolvers, res)
	}
	controller, err := bufcli2.NewController(container)
	if err != nil {
		return err
	}
	for _, schema := range f.Schemas {
		...
		resolvers = append(resolvers, bufcurl2.ResolverForImage(image))
	}
	...
	wktResolver, err := bufcurl2.NewWKTResolver(ctx, container.Logger())
	if err != nil {
		return err
	}
	res := bufcurl2.CombineResolvers(append(resolvers, wktResolver)...)
	...
	case f.ListServices || f.ListMethods:
		...
		for _, serviceName := range serviceNames {
			if f.ListServices {
				...
				serviceDescriptor, err := bufcurl2.ResolveServiceDescriptor(res, string(serviceName))
				if err != nil {
					return err
				}
	...
	default:
		// Invoke RPC
		methodDescriptor, err := bufcurl2.ResolveMethodDescriptor(res, service, method)
		if err != nil {
			return err
		}
		transport, err := makeTransportOnce()
		...
		invoker := bufcurl2.NewInvoker(container, verbosePrinter, methodDescriptor, res, f.EmitDefaults, transport, clientOptions, urlArg, output)
