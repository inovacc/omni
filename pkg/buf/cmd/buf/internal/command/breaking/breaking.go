...
func run(
	...
	imageWithConfigs, checkClient, err := controller.GetTargetImageWithConfigsAndCheckClient(
		...
		input,
		wasmRuntime,
		bufctl.WithImageTargetPaths(flags.Paths, flags.ExcludePaths, pathsFlagName, excludePathsFlagName),
		bufctl.WithImageConfigOverride(flags.Config),
	)
	if err != nil {
		return err
	}
	...
	if flags.Against != "" {
		...
		againstImagesWithConfigs, _, err := controller.GetTargetImageWithConfigsAndCheckClient(
			...
			bufctl.WithImageTargetPaths(externalPaths, flags.ExcludePaths, pathsFlagName, excludePathsFlagName),
			bufctl.WithImageConfigOverride(flags.AgainstConfig),
		)
		if err != nil {
			return err
		}
