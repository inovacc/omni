...
` +
			bufcli2.GetInputLong(`the source, module, or image to check for breaking changes`),
...
func run(
	...
	controller, err := bufcli2.NewController(
		...
		bufctl2.WithFileAnnotationErrorFormat(flags.ErrorFormat),
	...
	if flags.Against != "" {
		...
		againstImagesWithConfigs, _, err := controller.GetTargetImageWithConfigsAndCheckClient(
			...
			bufctl2.WithTargetPaths(externalPaths, flags.ExcludePaths),
			bufctl2.WithConfigOverride(flags.AgainstConfig),
		)
		if err != nil {
			return err
		}
		...
		againstImages, err = xslices.MapError(
			...
			func(imageWithConfig bufctl2.ImageWithConfig) (bufimage.Image, error) {
	...
	allCheckConfigs := make([]bufconfig2.CheckConfig, 0, len(imageWithConfigs)*2)
	for _, imageWithConfig := range imageWithConfigs {
...
func hasNoUniqueBreakingConfig(imageWithConfigs []bufctl2.ImageWithConfig) bool {
	var base bufconfig2.BreakingConfig
	for _, imageWithConfig := range imageWithConfigs {
		if base == nil {
...
func equalBreakingConfig(breakingConfig1, breakingConfig2 bufconfig2.BreakingConfig) bool {
...
func filterImageWithConfigsNotInAgainstImages(
	imageWithConfigs []bufctl2.ImageWithConfig,
	againstImages []bufimage.Image,
) ([]bufctl2.ImageWithConfig, error) {
	foundAgainstImageIndices := make(map[int]struct{})
	var filteredImageWithConfigs []bufctl2.ImageWithConfig
	for _, imageWithConfig := range imageWithConfigs {
