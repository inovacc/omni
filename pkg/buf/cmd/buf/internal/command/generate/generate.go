...
func getInputImages(
	...
	if inputSpecified != "" || len(bufGenYAMLFile.InputConfigs()) == 0 {
		...
		inputImage, err := controller.GetImage(
			ctx,
			input,
			bufctl2.WithConfigOverride(moduleConfigOverride),
			bufctl2.WithTargetPaths(targetPathsOverride, excludePathsOverride),
			bufctl2.WithImageIncludeTypes(includeTypes),
			bufctl2.WithImageExcludeTypes(excludeTypes),
		)
		if err != nil {
			return nil, err
		}
	...
	for _, inputConfig := range bufGenYAMLFile.InputConfigs() {
		...
		inputImage, err := controller.GetImageForInputConfig(
			...
			inputConfig,
			bufctl2.WithConfigOverride(moduleConfigOverride),
			bufctl2.WithTargetPaths(targetPaths, excludePaths),
			bufctl2.WithImageIncludeTypes(includeTypes),
			bufctl2.WithImageExcludeTypes(excludeTypes),
		)
		if err != nil {
