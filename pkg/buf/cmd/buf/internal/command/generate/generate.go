...
func run(
	...
	if flags.IncludeWKTOverride != nil &&
		*flags.IncludeWKTOverride &&
		...
		return appcmd2.NewInvalidArgumentErrorf("Cannot set --%s to true without setting --%s to true", includeWKTFlagName, includeImportsFlagName)
	}
...
func getInputImages(
	...
	if inputSpecified != "" || len(bufGenYAMLFile.InputConfigs()) == 0 {
		...
		inputImage, err := controller.GetImage(
			ctx,
			input,
			bufctl.WithImageConfigOverride(moduleConfigOverride),
			bufctl.WithImageTargetPaths(targetPathsOverride, excludePathsOverride, pathsFlagName, excludePathsFlagName),
	...
	for _, inputConfig := range bufGenYAMLFile.InputConfigs() {
		...
		inputImage, err := controller.GetImageForInputConfig(
			...
			inputConfig,
			bufctl.WithImageConfigOverride(moduleConfigOverride),
			bufctl.WithImageTargetPaths(targetPaths, excludePaths, pathsFlagName, excludePathsFlagName),
			bufctl.WithImageIncludeTypes(includeTypes),
			bufctl.WithImageExcludeTypes(excludeTypes),
		)
		if err != nil {
