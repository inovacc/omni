...
import (
	...
	"github.com/inovacc/omni/pkg/buf/pkg/app/appcmd/appcmdtesting"
	...
	"github.com/inovacc/omni/pkg/buf/pkg/standard/xtesting"
...
`),
	...
	diff, err := storage2.DiffBytes(context.Background(), expected, actual)
	require.NoError(t, err)
...
func testGenerateInsertionPoint(
	...
	require.NoError(t, err)
	diff, err := storage2.DiffBytes(context.Background(), expectedOutput, readWriteBucket)
	require.NoError(t, err)
	require.Empty(t, string(diff))
}
...
func transformGolangProtocVersionToUnknown(t *testing.T) storage2.DiffOption {
	return storage2.DiffWithTransform(func(_, _ string, content []byte) []byte {
