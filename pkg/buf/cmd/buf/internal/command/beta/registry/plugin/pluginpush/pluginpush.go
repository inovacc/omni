...
import (
	...
	bufremoteplugindocker2 "github.com/inovacc/omni/pkg/buf/internal/bufpkg/bufremoteplugin/bufremoteplugindocker"
...
func NewCommand(
	...
	return &appcmd.Command{
		...
		Long:  bufcli2.GetSourceDirLong(`the source to push (directory containing buf.plugin.yaml or plugin release zip)`),
...
func run(
	...
	bufcli2.WarnBetaCommand(ctx, container)
	...
	source, err := bufcli2.GetInputValue(container, "" /* The input hashtag is not supported here */, ".")
	if err != nil {
		return err
	}
	...
	var sourceBucket storage2.ReadWriteBucket
	...
	client, err := bufremoteplugindocker2.NewClient(container.Logger(), bufcli2.Version)
	if err != nil {
		return err
	}
	defer func() {
	...
	if flags.Image != "" {
		...
		defer func() {
			if err := image.Close(); err != nil && !errors.Is(err, storage2.ErrClosed) {
	...
	authConfig := &bufremoteplugindocker2.RegistryAuthConfig{}
	if machine != nil {
...
func pushImage(
	ctx context.Context,
	client bufremoteplugindocker2.Client,
	authConfig *bufremoteplugindocker2.RegistryAuthConfig,
...
func unzipPluginToSourceBucket(ctx context.Context, pluginZip string, size int64, bucket storage2.ReadWriteBucket) (retErr error) {
...
func loadDockerImage(ctx context.Context, bucket storage2.ReadBucket) (storage2.ReadObjectCloser, error) {
	image, err := bucket.Get(ctx, bufremoteplugindocker2.ImagePath)
	if errors.Is(err, fs.ErrNotExist) {
		return nil, fmt.Errorf("unable to find a %s plugin image: %w", bufremoteplugindocker2.ImagePath, err)
	}
	return image, nil
}
