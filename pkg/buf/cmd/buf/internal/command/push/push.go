...
func NewCommand(
	...
	return &appcmd.Command{
		...
		Long:  bufcli2.GetSourceLong(`the source to push`),
...
func run(
	...
	uploader, err := bufcli2.NewModuleUploader(container)
	if err != nil {
		return err
	}

	var uploadOptions []bufmodule2.UploadOption
	if flags.GitMetadata {
		...
		if len(flags.Labels) > 0 {
			uploadOptions = append(uploadOptions, bufmodule2.UploadWithLabels(xslices.ToUniqueSorted(flags.Labels)...))
		}
	} else {
	...
	if flags.Create {
		createModuleVisibility, err := bufmodule2.ParseModuleVisibility(flags.CreateVisibility)
		if err != nil {
			return err
		}
		uploadOptions = append(
			uploadOptions,
			bufmodule2.UploadWithCreateIfNotExist(createModuleVisibility, flags.CreateDefaultLabel),
		)
	}
	if flags.SourceControlURL != "" {
		uploadOptions = append(uploadOptions, bufmodule2.UploadWithSourceControlURL(flags.SourceControlURL))
	}
	if flags.ExcludeUnnamed {
		uploadOptions = append(uploadOptions, bufmodule2.UploadWithExcludeUnnamed())
	}
...
func getBuildableWorkspace(
	...
	source, err := bufcli2.GetInputValue(container, flags.InputHashtag, ".")
	if err != nil {
		return nil, err
	}
...
func validateCreateFlags(flags *flags) error {
	if flags.Create {
		...
		if _, err := bufmodule2.ParseModuleVisibility(flags.CreateVisibility); err != nil {
...
func getGitMetadataUploadOptions(
	...
	var gitMetadataUploadOptions []bufmodule2.UploadOption
	...
	if flags.SourceControlURL == "" {
		...
		gitMetadataUploadOptions = append(gitMetadataUploadOptions, bufmodule2.UploadWithSourceControlURL(sourceControlURL))
	}
	...
	if flags.Create && flags.CreateDefaultLabel == "" {
		...
		gitMetadataUploadOptions = append(
			...
			bufmodule2.UploadWithCreateIfNotExist(createModuleVisibility, originRemote.HEADBranch()),
		)
	}
...
func getGitMetadataLabelsUploadOptions(
	...
	return bufmodule2.UploadWithLabels(refs...), nil
}

func getLabelUploadOption(flags *flags) bufmodule2.UploadOption {
	...
	// flags are set.
	if len(flags.Labels) > 0 {
		return bufmodule2.UploadWithLabels(xslices.ToUniqueSorted(flags.Labels)...)
	}
	if len(flags.Tags) > 0 {
		return bufmodule2.UploadWithTags(xslices.ToUniqueSorted(flags.Tags)...)
	}
	if flags.Branch != "" {
		// We upload to a single label, the branch name.
		return bufmodule2.UploadWithLabels(flags.Branch)
	}
	if flags.Draft != "" {
		// We upload to a single label, the draft name.
		return bufmodule2.UploadWithLabels(flags.Draft)
	}
	return nil
}
