...
func validateCreateFlags(flags *flags) error {
	if flags.Create {
		if flags.CreateVisibility == "" {
			return appcmd2.NewInvalidArgumentErrorf(
...
func validateLabelFlagCombinations(flags *flags) error {
	...
	if len(usedFlags) > 1 {
		...
		return appcmd2.NewInvalidArgumentErrorf("These flags cannot be used in combination with one another: %s", usedFlagsErrStr)
	}
	return nil
}
...
func validateLabelFlagValues(flags *flags) error {
	if slices.Contains(flags.Labels, "") {
		return appcmd2.NewInvalidArgumentErrorf("--%s requires a non-empty string", labelFlagName)
	}
	if slices.Contains(flags.Tags, "") {
		return appcmd2.NewInvalidArgumentErrorf("--%s requires a non-empty string", tagFlagName)
	}
	return nil
}
...
func getGitMetadataUploadOptions(
	...
	if err != nil {
		if errors.Is(err, git.ErrRemoteNotFound) {
			return nil, appcmd2.NewInvalidArgumentErrorf("remote %s must be present on Git checkout: %w", gitOriginRemote, err)
		}
		return nil, err
	}
	...
	if flags.SourceControlURL == "" {
		...
		if sourceControlURL == "" {
			return nil, appcmd2.NewInvalidArgumentError("unable to determine source control URL for this repository; only GitHub/GitLab/BitBucket are supported")
		}
...
func validateInputIsValidDirAndGitCheckout(
	...
	if _, err := buffetch.NewDirRefParser(container.Logger()).GetDirRef(ctx, input); err != nil {
		return appcmd2.NewInvalidArgumentErrorf("input %q is not a valid directory: %w", input, err)
	}
	if err := git.CheckDirectoryIsValidGitCheckout(ctx, container, input); err != nil {
		if errors.Is(err, git.ErrInvalidGitCheckout) {
			return appcmd2.NewInvalidArgumentErrorf("input %q is not a local Git repository checkout", input)
		}
		return err
	}
	return nil
