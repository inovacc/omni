...
func NewCommand(
	...
	return &appcmd.Command{
		...
		Long: bufcli2.GetSourceOrModuleLong(`the source or module to export`) + `
...
func run(
	...
	moduleReadBucket := bufmodule2.ModuleSetToModuleReadBucketWithOnlyProtoFiles(workspace)
	...
	if flags.All {
		...
		targetModules := bufmodule2.ModuleSetTargetModules(workspace)
		for _, module := range targetModules {
			docFile, err := bufmodule2.GetDocFile(ctx, module)
			...
			licenseFile, err := bufmodule2.GetLicenseFile(ctx, module)
			// If the file is not found, then we ignore it.
			if err != nil && !errors.Is(err, fs.ErrNotExist) {
				return err
			}
	...
	if flags.ExcludeImports {
		if err := moduleReadBucket.WalkFileInfos(
			ctx,
			func(fileInfo bufmodule2.FileInfo) error {
			...
			bufmodule2.WalkFileInfosWithOnlyTargetFiles(),
		); err != nil {
			return err
		}
		return nil
