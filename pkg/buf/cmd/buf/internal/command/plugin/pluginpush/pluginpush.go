...
func validateTypeFlags(flags *flags) error {
	...
	if len(typeFlags) == 0 {
		return appcmd2.NewInvalidArgumentErrorf("--%s must be set", binaryFlagName)
	}
	return nil
}
...
func validateCreateFlags(flags *flags) error {
	...
	if flags.Create {
		if flags.CreateType == "" {
			return appcmd2.NewInvalidArgumentErrorf("--%s must be set if --%s is set", createTypeFlagName, createFlagName)
		}
