...
func (f *flags) Bind(flagSet *pflag.FlagSet) {
	bufcli2.BindVisibility(flagSet, &f.Visibility, visibilityFlagName, false)
	flagSet.StringVar(
		&f.Format,
		formatFlagName,
...
func run(
	...
	visibility, err := bufcli2.VisibilityFlagToPolicyVisibilityAllowUnspecified(flags.Visibility)
	if err != nil {
		return appcmd.WrapInvalidArgumentError(err)
	}
	...
	if err != nil {
		if connect.CodeOf(err) == connect.CodeAlreadyExists {
			return bufcli2.NewPolicyNameAlreadyExistsError(policyFullName.String())
		}
		return err
	}
