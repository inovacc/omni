...
func (f *flags) Bind(flagSet *pflag.FlagSet) {
	bufcli2.BindArchiveStatus(flagSet, &f.ArchiveStatus, archiveStatusName)
	flagSet.Uint32Var(
		&f.PageSize,
		pageSizeFlagName,
		defaultPageSize,
...
func run(
	...
	archiveStatusFilter, err := bufcli2.ArchiveStatusFlagToPluginArchiveStatusFilter(flags.ArchiveStatus)
	if err != nil {
		return appcmd.WrapInvalidArgumentError(err)
	}
	...
	if err != nil {
		if connect.CodeOf(err) == connect.CodeNotFound {
			return bufcli2.NewRefNotFoundError(pluginRef)
		}
		return err
	}
...
func nextPageCommand(container appext.Container, flags *flags, nextPageToken string) string {
	...
	if flags.ArchiveStatus != bufcli2.DefaultArchiveStatus {
