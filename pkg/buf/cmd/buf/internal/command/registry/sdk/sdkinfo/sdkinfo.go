...
func (f *flags) Bind(flagSet *pflag.FlagSet) {
	...
	_ = appcmd2.MarkFlagRequired(flagSet, moduleFlagName)
	_ = appcmd2.MarkFlagRequired(flagSet, pluginFlagName)
}
...
func run(
	...
	if moduleRef.FullName().Registry() != pluginIdentity.Remote() {
		return appcmd2.NewInvalidArgumentError("module and plugin must be from the same BSR instance")
	}
