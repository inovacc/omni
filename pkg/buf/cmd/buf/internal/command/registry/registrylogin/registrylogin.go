...
import (
	...
	oauth3 "github.com/inovacc/omni/pkg/buf/pkg/oauth2"
...
func inner(
	...
	if flags.TokenStdin && flags.Prompt {
		return appcmd2.NewInvalidArgumentErrorf("cannot use both --%s and --%s flags", tokenStdinFlagName, promptFlagName)
	}
	var token string
	if flags.TokenStdin {
