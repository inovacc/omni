...
import (
	...
	oauth3 "github.com/inovacc/omni/pkg/buf/internal/pkg/oauth2"
...
func inner(
	...
	clientConfig, err := bufcli2.NewConnectClientConfigWithToken(container, token)
	if err != nil {
		return err
	}
...
func doPromptLogin(
	...
	var err error
	token, err := bufcli2.PromptUserForPassword(container, "Token: ")
	if err != nil {
		if errors.Is(err, bufcli2.ErrNotATTY) {
...
func doBrowserLogin(
	...
	oauth2Client := oauth3.NewClient(baseURL, client)
	// Register the device.
	deviceRegistration, err := oauth2Client.RegisterDevice(ctx, &oauth3.DeviceRegistrationRequest{
		ClientName: clientName,
	})
	if err != nil {
		var oauth2Err *oauth3.Error
	...
	deviceAuthorization, err := oauth2Client.AuthorizeDevice(ctx, &oauth3.DeviceAuthorizationRequest{
...
`,
	...
	deviceToken, err := oauth2Client.AccessDeviceToken(ctx, &oauth3.DeviceAccessTokenRequest{
		ClientID:     deviceRegistration.ClientID,
		ClientSecret: deviceRegistration.ClientSecret,
		DeviceCode:   deviceAuthorization.DeviceCode,
		GrantType:    oauth3.DeviceAuthorizationGrantType,
	}, oauth3.AccessDeviceTokenWithPollingInterval(time.Duration(deviceAuthorization.Interval)*time.Second))
	if err != nil {
		var oauth2Err *oauth3.Error
		if errors.As(err, &oauth2Err) {
			return "", fmt.Errorf("authorization failed: %s", oauth2Err.ErrorDescription)
