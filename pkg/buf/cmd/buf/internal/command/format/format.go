...
func run(
	...
	moduleReadBucket := bufmodule2.ModuleReadBucketWithOnlyTargetFiles(
		...
		bufmodule2.ModuleSetToModuleReadBucketWithOnlyProtoFilesForTargetModules(workspace),
	)
	originalReadBucket := bufmodule2.ModuleReadBucketToStorageReadBucket(moduleReadBucket)
	...
	defer func() {
		if retErr == nil && flags.ExitCode && diffExists {
			retErr = bufctl2.ErrFileAnnotation
		}
	}()
