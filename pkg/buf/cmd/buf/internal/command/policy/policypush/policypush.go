...
func upload(
	...
	container appext.Container,
	flags *flags,
) (_ bufpolicy2.Commit, retErr error) {
	var policy bufpolicy2.Policy
	...
	policy, err = bufpolicy2.NewPolicy(policyFilePath, policyFullName, policyFullName.Name(), uuid.Nil, policyYamlFile.PolicyConfig)
	if err != nil {
		return nil, fmt.Errorf("unable to create policy from file %q: %w", policyFilePath, err)
	}
	uploader, err := bufcli2.NewPolicyUploader(container)
	if err != nil {
		return nil, err
	}
	var options []bufpolicy2.UploadOption
	if flags.Create {
		createPolicyVisibility, err := bufpolicy2.ParsePolicyVisibility(flags.CreateVisibility)
		if err != nil {
			return nil, err
		}
		options = append(options, bufpolicy2.UploadWithCreateIfNotExist(
			createPolicyVisibility,
		))
	}
	if len(flags.Labels) > 0 {
		options = append(options, bufpolicy2.UploadWithLabels(flags.Labels...))
	}
	if flags.SourceControlURL != "" {
		options = append(options, bufpolicy2.UploadWithSourceControlURL(flags.SourceControlURL))
	}
	commits, err := uploader.Upload(ctx, []bufpolicy2.Policy{policy}, options...)
	if err != nil {
		return nil, err
	}
	if len(commits) != 1 {
...
func validateCreateFlags(flags *flags) error {
	if flags.Create {
		...
		if _, err := bufpolicy2.ParsePolicyVisibility(flags.CreateVisibility); err != nil {
