...
import (
	...
	"github.com/inovacc/omni/pkg/buf/internal/buf/bufpolicy/bufpolicyconfig"
)
...
func upload(
	...
	if err != nil {
		return nil, appcmd2.NewInvalidArgumentErrorf("could not read policy file %q: %w", policyFilePath, err)
	}
	...
	if err != nil {
		return nil, appcmd2.NewInvalidArgumentErrorf("unable to validate policy file %q: %w", policyFilePath, err)
	}
	// We parse the policy full name from the user-provided argument.
	if policyYamlFile.Name() == "" {
		return nil, appcmd2.NewInvalidArgumentErrorf("policy file %q must have a name", policyFilePath)
	}
	policyFullName, err := bufparse.ParseFullName(policyYamlFile.Name())
	if err != nil {
		return nil, appcmd2.NewInvalidArgumentErrorf("unable to parse policy full name %q: %w", policyYamlFile.Name(), err)
	}
...
func validateCreateFlags(flags *flags) error {
	if flags.Create {
		if flags.CreateVisibility == "" {
			return appcmd2.NewInvalidArgumentErrorf("--%s must be set if --%s is set", createVisibilityFlagName, createFlagName)
		}
