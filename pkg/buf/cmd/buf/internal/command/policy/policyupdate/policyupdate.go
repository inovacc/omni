...
func run(
	...
	policyKeyProvider, err := bufcli2.NewPolicyKeyProvider(container)
	if err != nil {
		return err
	}
	...
	var configuredPolicyNameToRemotePluginKeys map[string][]bufplugin2.PluginKey
	if policyCount := len(configuredRemotePolicyNameToRemotePluginKeys) + len(configuredLocalPolicyNameToRemotePluginKeys); policyCount > 0 {
		configuredPolicyNameToRemotePluginKeys = make(map[string][]bufplugin2.PluginKey, policyCount)
...
func getPolicyKeyPluginKeysForPolicyKeys(
...
) (map[string][]bufplugin2.PluginKey, error) {
	if len(policyKeys) == 0 {
		return nil, nil
	}
	policyDataProvider, err := bufcli2.NewPolicyDataProvider(container)
	if err != nil {
		return nil, err
	}
	pluginKeyProvider, err := bufcli2.NewPluginKeyProvider(container)
	if err != nil {
		return nil, err
	}
	...
	policyNameToRemotePluginKeys := make(map[string][]bufplugin2.PluginKey)
	for _, policyData := range policyDatas {
		...
		pluginRefs, err := xslices.MapError(pluginConfigs, func(pluginConfig bufpolicy2.PluginConfig) (bufparse.Ref, error) {
