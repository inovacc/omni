...
// Run executes the ls command with the given options
...
func fileInfoToEntry(path string, info fs.FileInfo) Entry {
	entry := Entry{
		Name:    info.Name(),
		Size:    info.Size(),
...
func printEntries(w io.Writer, entries []Entry, opts Options) {
	if opts.LongFormat {
...
func printLongFormat(w io.Writer, entries []Entry, opts Options) {
...
func printOnePerLine(w io.Writer, entries []Entry, opts Options) {
	for _, e := range entries {
		name := e.Name
		if opts.Classify {
			name += classifyIndicator(e)
...
func printSimple(w io.Writer, entries []Entry, opts Options) {
...
func classifyIndicator(e Entry) string {
	if e.IsDir {
		return "/"
	}
...
// List returns a list of file names in the given path (simple version for compatibility)
